// 注視データを他ファイルで扱いやすいように加工しfixation_list.csvに書き込みを行うプログラム
#include<stdio.h>
#include<string.h>
...
int main() {
    ...
    if ((memfp = fopen(member_file_name, "r")) == NULL) {
        fprintf(stderr, "csvファイルのオープンに失敗しました．\n");
        return -1;
    }

    // 解析結果を書き込みcsvファイルとストリームの指定
    FILE *resfp;
    char result_file_name[] = "fixation_list.csv";

    i = 0;
    while (fscanf(memfp, "%lf,%c", &member[i].grade, &member[i].class) != EOF) {
        i++;
    }
    csv_data_count = i;
    ...
    fprintf(resfp, "number,fixation_start,fixation_time,fixation_object,class\n");
    ...
    for (i = 1; i <= csv_data_count; i++) {
        ...
        for (k = 0; k < fixation_count; k++) fprintf(resfp, "%d,%d,%d,%d,%c\n", i, start_time_list[k], fixation_time_list[k], fixation_position_list[k], member[i-1].class);
        // fprintf(resfp, "\n");
